  ...
  <div class="container-fluid conatainer-px">
    <nav aria-label="breadcrumb" class="py-2 mt-3">
      <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item">
          <a href="#" class="text-decoration-none" @click.prevent="breadCrumbSearch('food', 'all')">品嚐美食</a>
        </li>
        <li class="breadcrumb-item" v-if="currentCity !== ''">
          <a href="#" class="text-decoration-none" @click.prevent="breadCrumbSearch('food', currentCity_en)">{{ currentCity }}</a>
        </li>
    ...
    <div class="row bg-md-gray">
      <div class="col-md-6">
        <ul class="contact">
          <li><strong>開放時間：</strong> : {{ detailData.OpenTime   }}</li>
          <li v-if="detailData.Phone"><strong>服務電話：</strong> : {{ detailData.Phone   }}</li>
          <li v-if="detailData.Address"><strong>景點地址：</strong> : {{ detailData.Address  }}</li>
          <li v-if="detailData.WebsiteUrl"><strong>官方網站：</strong> : <a :href="detailData.WebsiteUrl" target="_blank">{{ detailData.WebsiteUrl }}</a></li>
          <li v-if="detailData.TicketInfo"><strong>票價資訊：</strong> : {{ detailData.TicketInfo   }}</li>
    ...
    <div class="mt-4">
      <div class="d-flex align-items-center">
        <h2 class="me-auto ms-2 d-md-block d-none">還有這些不能錯過的美食</h2>
        <h2 class="me-auto ms-2 d-block d-md-none">這些也不能錯過</h2>
        <a class="more-link me-2 d-md-block d-none" href="#"
        @click.prevent="goToRelation('food')"
          >查看更多{{ this.currentCity }}美食<img
...
export default {
  ...
  methods: {
    ...
    getDetail () {
      this.axios.get(`https://ptx.transportdata.tw/MOTC/v2/Tourism/Restaurant?$filter=RestaurantID%20eq%20'${this.id}'&$top=1&$format=JSON`,
        {
    ...
    getAroundFood () {
      this.axios.get(`https://ptx.transportdata.tw/MOTC/v2/Tourism/Restaurant?$filter=Picture%2FPictureUrl1%20ne%20null%20and%20RestaurantID%20ne%20'${this.detailData.ID}'&$orderby=UpdateTime%20desc&$top=4&$spatialFilter=nearby(${this.detailData.Position.PositionLat}%2C%20${this.detailData.Position.PositionLon}%2C%2010000)&$format=JSON`,
        {
          headers: this.$getAuthorizationHeader()
        }
      )
    ...
    renderCarousel (data) {
      ...
      str += contentStr
      str += NavbtnStr
      this.$refs.carouselFood.innerHTML = str
    },
    getCityList () {
